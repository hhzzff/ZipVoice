...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        参数:
            d_model (int): 特征的维度 (即 text_x 的 embedding_dim)。
            dropout (float): Dropout 概率，常用于防止过拟合，一般设为 0.1。
            max_len (int): 预计算的最大序列长度。如果你的文本序列通常不超过 5000 个 Token，保留默认值即可。
        """
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)

        # 预先计算好所有的位置编码矩阵
        # pe 的形状将是: (max_len, 1, d_model)
        pe = torch.zeros(max_len, 1, d_model)
        
        # position: (max_len, 1) 代表位置索引 0, 1, 2 ... max_len-1
        position = torch.arange(max_len, dtype=torch.float).unsqueeze(1)
        
        # div_term: 计算 sin 和 cos 内部的缩放因子
        ...
        # 将 pe 注册为模型的 buffer 而不是可训练的 parameter (不需要计算梯度)。
        # 这样在模型保存 (state_dict) 和设备移动 (model.to('cuda')) 时，pe 会自动跟随。
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        参数:
            x (Tensor): 文本特征输入，形状通常为 [Seq_Len, Batch_Size, d_model]
            
        返回:
            注入了位置信息后的文本特征张量
        """
        # x.size(0) 是当前的实际序列长度 T
        # 取出预计算好的前 T 个位置编码，并通过广播机制加到整个 batch 上
        # self.pe[:x.size(0)] 的形状是 (T, 1, d_model)，加法会自动广播到 (T, Batch, d_model)
        x = x + self.pe[:x.size(0)]
        
        return self.dropout(x)


class TTSAudioZipformer(nn.Module):
    """
    Args:
    ...
    def __init__(
        ...
        super(TTSAudioZipformer, self).__init__()
        ...
        self.text_in_proj = nn.Linear(in_dim // 2, encoder_dim)
        self.text_pos_encoder = PositionalEncoding(d_model=encoder_dim, dropout=0.1)
    ...
    def forward(
        ...
        Args:
          x:
            The speech input tensor. Its shape is (batch_size, seq_len, feat_dim * 2).
          text_x:
            The text input tensor. Its shape is (batch_size, seq_len, feat_dim).
          t:
        ...
        x = self.in_proj(x)
        text_x = self.text_in_proj(text_x)
        text_x = self.text_pos_encoder(text_x) 
        if t is not None:
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # 语音对文本的路由表 (不需要相对位置编码)
        self.cross_attn_weights = CrossMultiheadAttentionWeights(
            embed_dim, num_heads, query_head_dim
        )
        # 文本特征提取
        self.cross_attn = CrossAttention(
            embed_dim, num_heads, value_head_dim
        )
    ...
    def forward(
        ...
        Args:
          ...
          text_x: the text sequence (required):
            shape (seq_len, batch_size, embedding_dim).
        ...
        cross_w = self.cross_attn_weights(
            query=src, 
            key=text_x, 
            key_padding_mask=src_key_padding_mask,
            text_padding_mask=text_padding_mask,
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            assert key_padding_mask.shape == (
                batch_size,
                seq_len,
            ), f"key_padding_mask.shape:{key_padding_mask.shape} batch_size,seq_len:{batch_size,seq_len}"
...
class CrossMultiheadAttentionWeights(nn.Module):
    def __init__(
        self,
        embed_dim: int,
        num_heads: int,
    ...
    def forward(
        self, 
        query: Tensor,          # Speech/Noise 特征: (T_q, B, C)
        key: Tensor,            # Text 特征: (T_k, B, C)
        key_padding_mask: Optional[Tensor] = None,
        text_padding_mask: Optional[Tensor] = None,  # 文本的 Pad Mask
    ) -> Tensor:
        ...
        # q: (B, H, T_q, D_q)
        q = q.view(T_q, B, self.num_heads, self.query_head_dim).permute(1, 2, 0, 3)
        # k: (B, H, T_k, D_q)
        k = k.view(T_k, B, self.num_heads, self.query_head_dim).permute(1, 2, 0, 3)

        # 3. 计算缩放点积 Attention Scores
        # scores: (B, H, T_q, T_k)
        scale = 1.0 / math.sqrt(self.query_head_dim)
        # q @ k^T
        scores = torch.matmul(q, k.transpose(-2, -1)) * scale

        # 4. 处理 Text 的 Padding Mask (非常重要！防止语音关注到文本的 <PAD> 符号)
        if text_padding_mask is not None:
            # text_padding_mask shape: (B, T_k), 其中 True 表示是 padding 需要被忽略
            # 扩展为 (B, 1, 1, T_k) 以广播匹配 scores 的维度
            mask = text_padding_mask.unsqueeze(1).unsqueeze(2)
            scores = scores.masked_fill(mask, float("-inf"))

        # 5. Softmax 归一化得到最终概率权重
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.num_heads = num_heads
        self.value_head_dim = value_head_dim
        self.all_head_dim = num_heads * value_head_dim
    ...
    def forward(
        self, 
        text_x: Tensor,                # Text 特征: (T_k, B, C)
        cross_attn_weights: Tensor     # 上一步算出的权重: (B, H, T_q, T_k)
    ) -> Tensor:
        
        T_k, B, _ = text_x.size()
        
        # 从 weights 中动态获取 T_q (Speech 的长度)
        # cross_attn_weights shape: (B, H, T_q, T_k)
        T_q = cross_attn_weights.size(2)
        ...
        # 2. Reshape 和 Permute
        # v: (B, H, T_k, D_v)
        v = v.view(T_k, B, self.num_heads, self.value_head_dim).permute(1, 2, 0, 3)

        # 3. 根据 Attention 权重聚合 Value
        # cross_attn_weights: (B, H, T_q, T_k)
        # v: (B, H, T_k, D_v)
        # out: (B, H, T_q, D_v)
        out = torch.matmul(cross_attn_weights, v)

        # 4. 恢复张量形状为 (T_q, B, 汇总维度)
        # out: (B, T_q, H, D_v) -> (T_q, B, H * D_v)
        out = out.permute(2, 0, 1, 3).contiguous().view(T_q, B, self.all_head_dim)
...
class FeedforwardModule(nn.Module):
    """Feedforward module in TTSAudioZipformer model."""
