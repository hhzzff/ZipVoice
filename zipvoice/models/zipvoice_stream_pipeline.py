...
class ZipVoice(nn.Module):
    ...
    def condition_time_mask(
        ...
        mask_starts = (features_lens - mask_size).to(torch.int64)
    ...
    def forward(
        ...
        speech_condition_mask = self.condition_time_mask(
            features_lens=features_lens,
            mask_percent=(0.75, 0.95),
        ...
        text_condition = text_condition[padding_mask]
        
        speech_condition_mask = torch.zeros(features.shape[0], features.shape[1], dtype=torch.bool)
        speech_condition_mask[:, features_lens[0]:] = True
        speech_condition = torch.where(speech_condition_mask.unsqueeze(-1), 0, features)
    ...
    def sample(
        ...
        Args:
            ...
            noise: the intitial noise, with the shape (batch, seq_len, feat_dim).
            t: the time step, with the shape (batch, 1, 1).
            t_target: the target time step, with the shape (batch, 1, 1).
