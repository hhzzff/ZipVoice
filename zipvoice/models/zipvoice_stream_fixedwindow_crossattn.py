...
from zipvoice.models.modules.zipformer_crossattn import TTSAudioZipformer
...
class ZipVoice(nn.Module):
    ...
    def __init__(
        ...
        self.fm_decoder = TTSAudioZipformer(
            in_dim=feat_dim * 2,
            # in_dim=feat_dim * 4,
            out_dim=feat_dim,
    ...
    def forward_fm_decoder(
        ...
        xt = torch.cat([xt, speech_condition], dim=2)
        ...
        if guidance_scale is not None:
            ...
            vt = self.fm_decoder(
                x=xt, text_x=text_condition, t=t, padding_mask=padding_mask, guidance_scale=guidance_scale, text_padding_mask=text_padding_mask
            )
        else:
            vt = self.fm_decoder(x=xt, text_x=text_condition, t=t, padding_mask=padding_mask, text_padding_mask=text_padding_mask)
        return vt
    ...
    def forward_text_condition(
        ...
        text_condition = embed
        text_num_frames = int(tokens_lens.max()) + 1
        text_padding_mask = make_pad_mask(tokens_lens, max_len=text_num_frames)  # (B, S)
    ...
    def forward_text_train(
        ...
        text_condition, padding_mask, text_padding_mask = self.forward_text_condition(
    ...
    def forward_text_inference_ratio_duration(
        ...
        return text_condition, padding_mask, text_padding_mask, features_lens
    ...
    def forward(
        ...
        (text_condition, padding_mask,text_padding_mask) = self.forward_text_train(
            tokens=tokens,
            features_lens=features_lens,
        )
    ...
    def sample(
        ...
        if duration == "predict":
            ...
            text_condition, padding_mask, text_padding_mask = self.forward_text_inference_gt_duration(
                tokens=tokens,
                features_lens=features_lens,
                prompt_tokens=prompt_tokens,
                prompt_features_lens=prompt_features_lens,
