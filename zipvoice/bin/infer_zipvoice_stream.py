...
from zipvoice.models.zipvoice_stream import ZipVoice
...
def generate_sentence(
    ...
    text_stream = stream_chunk_text(text)
    pred_features_all = None
    output_wav = []
    for i, text_chunk_str in enumerate(text_stream):
        print(f"Processing chunk {i}: {text_chunk_str}, prompt_text: {prompt_text}")
        ...
        wav = (
            vocoder.decode(pred_features[:, :, : pred_features_lens[0]])
            .squeeze(1)
            .clamp(-1, 1)
        )
        ...
        torchaudio.save(f"{save_path}_chunk_{text_chunk_str}.wav", wav.cpu(), sample_rate=sampling_rate)
        output_wav.append(wav)

        # Finish model generation
        t = (dt.datetime.now() - start_t).total_seconds()
        prompt_text = prompt_text + " " + text_chunk_str
        prompt_features = torch.cat([prompt_features, pred_features.permute(0, 2, 1) * feat_scale], dim=1)
